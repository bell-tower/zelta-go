...
- **Send-check / feature drop (provisional)** — Go currently has an opt-in root `zfs send -nv` preflight that drops recognized unsupported defaults in oracle order `-e`, `-c`, `-L`; this is not considered parity until upstream behavior is corrected. NetBSD `recv -x` and encryption-feature mismatch cases are tracked in `AGENTS-TODO-upstream.md`; runtime retry after a failed stream remains deferred.
