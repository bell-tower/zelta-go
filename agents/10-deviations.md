...
- **No pure-Go SSH client** — Zelta shells out to OpenSSH (`ssh` binary). Does **not** use `golang.org/x/crypto/ssh` as a client (config, agent, mux, ProxyJump stay native). Sylve may use `x/crypto/ssh` only as its **cluster embedded server**; Sylve backup/replication clients also use the OpenSSH binary. See `agents/16-sdk.md` transport table.
