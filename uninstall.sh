...
is_in_git_repo() {
	...
	# Protect paths that are themselves a git work tree (or nested share/
	# of one), but not mere descendants of an unrelated checkout root.
	current="$check_path"
	while [ "$current" != "/" ] && [ "$current" != "." ]; do
		if [ -d "$current/.git" ]; then
			# Only treat as protected if the target is the repo root or a
			# first-level tree dir (bin, share, doc) — not tmp sandboxes.
			case "$check_path" in
				"$current"|"$current"/bin|"$current"/bin/*|\
				"$current"/share|"$current"/share/*|\
				"$current"/doc|"$current"/doc/*|\
				"$current"/cmd|"$current"/cmd/*) return 0 ;;
				*) return 1 ;;
			esac
		fi
